...
export function addBook(book = null, name = null, mediaType = null, length = null, read = null, completed = null) {
	const bookshelfShelfs = getBookshelfShelfs();
	const shelf = findOrCreateShelf(bookshelfShelfs);

	const newBook = createBookElement(book, name, mediaType, length, read, completed);
	shelf.appendChild(newBook);
}

export function updateBook(id, name = null, mediaType = null, length = null, read = null, completed = null) {
	if (id === null || id === "") return;

	// console.log(`ID = ${id}, NOME = ${name}, MEDIATYPE = ${mediaType}, LENGTH = ${length}, COMPLETED = ${completed}`);
...
function createBookElement(book, name, mediaType, length, read, completed) {
	...
	if (book === null) {
		...
		book = { name, mediaType, length, read, completed, ...book };
		saveBook(book);
	}
...
function addBookOpenEvent(newBook) {
	...
	function handleBookEvent(e) {
		...
		openBookForm(bookData.name, bookData.mediaType, bookData.length, bookData.read, bookData.completed, bookData.name, true, "written", bookData.id);
	}
}
...
function addBookRemoveEvent(newBook) {
	newBook.addEventListener("contextmenu", (e) => {
		...
		if (removeBookStage === 0 || (removeBookStage === 1 && e.target.id !== removeBookId)) {
